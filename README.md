...
> Password-Strength-Analyzer - Open source tool by AetherCodeHQ

`analysis` `cli` `golang` `password` `security`

## What is it?

**Password Strength Analyzer** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Password-Strength-Analyzer.git
cd Password-Strength-Analyzer
go build -o Password-Strength-Analyzer .
```

### Run
```bash
./Password-Strength-Analyzer --help
```
...
```bash
# Basic usage
./Password-Strength-Analyzer --path ./target

# With options
./Password-Strength-Analyzer --path ./target --format json --output report.json

# Verbose mode
./Password-Strength-Analyzer --path ./target --verbose
```
...
### Basic scan
```bash
./Password-Strength-Analyzer --path ./my-project
```

### JSON report
```bash
./Password-Strength-Analyzer --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Password Strength Analyzer
  run: ./Password-Strength-Analyzer --path . --format json --output report.json
```
